...
from ogb.utils.features import allowable_features, atom_to_feature_vector, safe_index


def bond_to_feature_vector(bond):
    """OGB bond features with a safe stereo fallback for newer RDKit enums."""
    return [
...
def mol_to_graph_data_obj_simple_3D_canonical(mol, pure_atomic_num=False):
    # Get Canonical order
    ranks = list(Chem.CanonicalRankAtoms(mol))
    ordered_atom_indices = sorted(range(mol.GetNumAtoms()), key=lambda x: ranks[x])

    # atoms - Reorder by Canonical order
    atom_features_list = []
    atom_count = defaultdict(int)
    for idx in ordered_atom_indices:  # use ordered_atom_indices rather than mol.GetAtoms()
    ...
    # positions - Reorder by Canonical order
    conformer = mol.GetConformers()[0]
    positions = [conformer.GetAtomPosition(i) for i in ordered_atom_indices]  # use ordered_atom_indices
    positions = torch.Tensor(positions)

    # bonds - Need to update the starting and ending atom indices of the bond to match the new order
    num_bond_features = 2  # bond type, bond direction
    if len(mol.GetBonds()) > 0:  # mol has bonds
        edges_list = []
        edge_feats_list = []
        # Create a mapping from old indices to new indices
        idx_map = {old_idx: new_idx for new_idx, old_idx in enumerate(ordered_atom_indices)}
        
        for bond in mol.GetBonds():
            i = idx_map[bond.GetBeginAtomIdx()]  # Map to new indices
            j = idx_map[bond.GetEndAtomIdx()]    # Map to new indices
            edge_feature = bond_to_feature_vector(bond)
            edges_list.append((i, j))
            edge_feats_list.append(edge_feature)
            edges_list.append((j, i))
