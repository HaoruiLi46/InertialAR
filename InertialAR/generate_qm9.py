...
from splitters import split_condition_ids_by_indices, split_qm9_indices
from model import InertialAR
from InertialTransformer.datasets import DatasetInertialSeqQM9
...
from InertialTransformer.datasets.functional_group_extract_qm9 import HEADERS
from InertialTransformer.datasets.functional_group_mapping import analyze_condition_distribution
...
def generate_batch(model, idx, positions, args, sample_logits=False):
    ...
    with torch.no_grad():
        ...
        try:
            ...
            print(f"Found atom mapping for dataset '{args.dataset}': {atom_map}")
            if atom_map is not None and len(atom_map) > 0:
                print(f"Using allowed token ids: {atom_map.keys()}")
                allowed_set = set(atom_map.keys()) | {int(eos_token_id)}
                allowed_token_ids = torch.tensor(sorted(allowed_set), device=device)
                print(f"Final allowed token ids: {allowed_token_ids}")
            else:
                print("No atom mapping found, using no token restrictions")
        except Exception as e:
            print(f"Error setting up token restrictions: {e}")
            allowed_token_ids = None
        for i in pbar:
            if has_stopped.all():
...
def sample_conditions_from_distribution(condition_distribution, num_samples, device='cpu', seed: int = 0):
    ...
    # Keep QM9 sampling on CPU with a local generator to avoid mutating global RNG state.
    weights_cpu = torch.tensor(weights, dtype=torch.float, device='cpu')

    # Use a local generator for deterministic sampling.
    gen = torch.Generator(device='cpu')
    gen.manual_seed(int(seed))

    # Sample weighted condition indices on CPU, then move them to the target device.
    sampled_indices = torch.multinomial(weights_cpu, num_samples, replacement=True, generator=gen)

    # Map sampled indices back to condition ids.
    condition_ids_cpu = torch.tensor(ids, dtype=torch.long, device='cpu')
    sampled_ids = condition_ids_cpu[sampled_indices]

    # Return shape: (num_samples, 1) on the target device.
    return sampled_ids.to(device).unsqueeze(1)
...
if __name__ == '__main__':
    ...
    data_root = os.path.join(args.data_root, args.dataset.upper())
    ...
    print(f"Dataset loaded successfully. Total size: {len(dataset)}")
    condition_ids = dataset.condition_ids
    train_idx, valid_idx, test_idx = split_qm9_indices(len(condition_ids), seed=args.seed)
    train_condition_ids, _, _ = split_condition_ids_by_indices(
        condition_ids, train_idx, valid_idx, test_idx
    )
    id_to_pattern_dict = dataset.id_to_pattern_dict
    pattern_to_id_dict = dataset.pattern_to_id_dict
    num_classes = dataset.num_classes
    condition_distribution = analyze_condition_distribution(
        train_condition_ids,
        HEADERS[:-1],
        id_to_pattern_dict,
