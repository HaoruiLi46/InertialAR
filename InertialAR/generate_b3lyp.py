...
from splitters import split_b3lyp_indices, split_condition_ids_by_indices
...
def sample(logits, temperature: float=1.0, top_k: int=0, top_p: float=1.0, sample_logits=True, allowed_token_ids=None):        
    ...
    # Optional: restrict logits to an allowed set (mask invalid tokens)
...
def generate_batch(model, idx, positions, args, sample_logits=False):
    ...
    with torch.no_grad():
        ...
        # Whitelist: only allow dataset-specific atom tokens (+EOS for stopping)
        ...
        try:
            ...
            allowed_token_ids = None
        # Use the same whitelist for all steps (no early-step exclusion)
        allowed_for_step = allowed_token_ids
        for i in pbar:
            if has_stopped.all():
            ...
            if i == 0:
                logits, z = model.inference(input_ids=None, positions=generated_positions, condition_id=generated_idx, allowed_token_ids=allowed_for_step)
            else:
                logits, z = model.inference(input_ids=generated_idx, positions=generated_positions, condition_id=cond_idx, allowed_token_ids=allowed_for_step)
            ...
            if args.cfg_scale > 1.0:
                ...
                next_token_ids, _ = sample(logits, top_k=top_k, top_p=top_p, temperature=temperature, sample_logits=False, allowed_token_ids=allowed_for_step)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default="B3LYP")
    ...
    train_idx, valid_idx, test_idx = split_b3lyp_indices(len(condition_ids), seed=args.seed)
    ...
    if total_generated_count == 0:
        ...
        print(f"Example command: python eval_transform.py --input_npz {output_file_path} --output_text {output_file_path_txt} --dataset_name geom")
        try:
            np.savez_compressed(
                output_file_path,
